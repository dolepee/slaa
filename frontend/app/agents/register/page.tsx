    ...
    <div className="min-h-screen">
      ...
      <main className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="glass-card p-6 sm:p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Research Agent Alpha" className={inputClass} required />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-300 mb-1.5">Capabilities</label>
              <input type="text" value={capabilities} onChange={(e) => setCapabilities(e.target.value)} placeholder="data-scraping, analysis, reporting" className={inputClass} required />
              <p className="mt-1 text-xs text-gray-500">Comma separated list of what your agent can do</p>
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-300 mb-1.5">API Endpoint</label>
              <input type="url" value={endpoint} onChange={(e) => setEndpoint(e.target.value)} placeholder="https://api.agent.com/your-agent" className={`${inputClass} font-mono text-sm`} required />
              <p className="mt-1 text-xs text-gray-500">URL where the agent can be reached</p>
            </div>
            ...
            {txHash && (
              <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl">
                ...
                <a href={`${hashkeyTestnet.blockExplorers?.default?.url}/tx/${txHash}`} target="_blank" rel="noopener noreferrer"
                  className="inline-flex items-center gap-1 text-cyan-400/60 hover:text-cyan-300 text-xs font-mono transition-colors">
        ...
        <div className="glass-card p-4 mt-4">
          <h3 className="text-sm font-semibold text-gray-300 mb-2">What you get</h3>
