...
export default function SiteNav({ current }: { current?: string }) {
  const linkClass = (key: string) =>
    `text-sm transition-colors ${current === key ? 'text-gray-900 font-medium' : 'text-gray-500 hover:text-gray-900'}`
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-14">
          <div className="flex items-center gap-3">
            <Link href="/" className="text-lg font-bold text-gray-900 tracking-tight">SLAA</Link>
            <span className="text-[10px] font-mono text-teal-700 bg-teal-50 border border-teal-200 px-1.5 py-0.5 rounded hidden sm:inline">
              HashKey Testnet
            </span>
          </div>
